...
#include "student.h"
  void splitList(CP::list<T> &list1, CP::list<T> &list2)
  {
    if (size() % 2 == 0)
    {
      ...
      node *prevStart1 = list1.mHeader->next;
      node *prevStart2 = list2.mHeader->next;
      list1.mHeader->next = start;
      start->prev = list1.mHeader;
      ...
      prevStart1->prev = start;
      list1.mSize += size() / 2;

      list2.mHeader->next = start;
      start->prev = list2.mHeader;
